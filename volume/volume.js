...
export async function init() {
   if (!(await Avatar.lang.addPluginPak('volume'))) {
      return infoOrange('volume: unable to load language pak files');
   }

   Locale = await Avatar.lang.getPak('volume', Config.language);
   if (!Locale) {
      return infoOrange(`volume: Unable to find the '${Config.language}' language pak.`);
   }

   periphInfo.push({
      Buttons: [
         {
            name: await Locale.get('label.title'),
            value_type: 'button',
            usage_name: 'Button',
            periph_id: '121212',
            notes: await Locale.get('label.notes'),
         },
      ],
   });
   return periphInfo;
...
export async function action(data, callback) {
   try {
      ...
      if (!data || !data.action || !data.action.command) {
         infoOrange('données invalides:', data);
         callback();
         return;
      }
      ...
      // Trace console
      infoGreen('volume: ', data.action.command, L.get('plugin.from'), data.client, L.get('plugin.to'), data.toClient);
   } catch (err) {
...
const controlVolume = async (data, allClients, type) => {
   ...
   switch (type) {
      ...
      case 'setUnmute':
         ...
         break;
      default:
         infoOrange(`volume: controlVolume -> type inconnu: ${type}`);
         return;
   }
